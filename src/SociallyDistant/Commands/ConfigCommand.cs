...
namespace SociallyDistant.Core.Commands
{
    public class ConfigurationManagerCommand : Command
    {
        ...
        public override string Description => "redwm ConfigurationManageruration utility.";
...
";
        ...
        {
            var res = ConfigurationManager.ActiveConfig.Resolution;
        ...
        private bool SetResolution(string value)
        {
            ...
            {
                ConfigurationManager.ActiveConfig.Resolution = string.Empty;
                return true;
            }

            try
            {
                ConfigurationManager.SetDisplayMode(value);
                return true;
            }
            catch (Exception ex)
            {
        ...
        private void RegisterSettings()
        {
            ...
            RegisterSetting(
                ...
                () => GetBoolean(ref ConfigurationManager.ActiveConfig.IsFullscreen),
                x => SetBoolean(ref ConfigurationManager.ActiveConfig.IsFullscreen, x)
            );
            
            RegisterSetting(
                "wm.vsync",
                "Synchronize on vertical re-trace (prevent screen-tearing and stabilize frame-rate during redwm render)",
                () => GetBoolean(ref ConfigurationManager.ActiveConfig.VSync),
                x => SetBoolean(ref ConfigurationManager.ActiveConfig.VSync, x)
            );
            
            RegisterSetting(
                "wm.fixedTimeStep",
                "Use fixed time-stepping for redwm updates",
                () => GetBoolean(ref ConfigurationManager.ActiveConfig.FixedTimeStepping),
                x => SetBoolean(ref ConfigurationManager.ActiveConfig.FixedTimeStepping, x)
            );

            RegisterSetting(
                "input.swapMouse",
                "Swap primary and secondary mouse buttons (right- or left-handed mode)",
                () => GetBoolean(ref ConfigurationManager.ActiveConfig.SwapMouseButtons),
                x => SetBoolean(ref ConfigurationManager.ActiveConfig.SwapMouseButtons, x)
            );
            
            RegisterSetting(
                "effects.bloom",
                "Enable/disable bloom effect (glowing text, saturated colors, stoner vision)",
                () => GetBoolean(ref ConfigurationManager.ActiveConfig.Effects.Bloom),
                x => SetBoolean(ref ConfigurationManager.ActiveConfig.Effects.Bloom, x)
            );
            
            RegisterSetting(
                "effects.shadowmask",
                "Enable/disable Cathode Ray Tube Shadow-mask effect",
                () => GetBoolean(ref ConfigurationManager.ActiveConfig.Effects.ShadowMask),
                x => SetBoolean(ref ConfigurationManager.ActiveConfig.Effects.ShadowMask, x)
            );
        }
        ...
        protected override void Main(string[] args)
        {
            ...
            if (action == "display-modes")
            {
                ...
                foreach (var mode in GamePlatform.GetAvailableDisplayModes(ConfigurationManager.ActiveConfig.Monitor))
                {
            ...
            if (action == "set")
            {
                ...
                {
                    ConfigurationManager.DiscardChanges();
